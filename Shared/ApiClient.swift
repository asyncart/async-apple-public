...
class ApiClient {
    static var shared: ApiClient = ApiClient(baseURL: Globals.API_BASE_URL)! {
        willSet {
            shared.invalidate()
        }
    }
    ...
    func getMarket(page: Int = 1, completionHandler: @escaping ([String: Any]?, Error?) -> Void) -> URLSessionTask {
        return GET(path: "/arts", params: ["page": String(describing: page)], completionHandler: completionHandler)
    }

    func getTags(completionHandler: @escaping ([String: Any]?, Error?) -> Void) -> URLSessionTask {
        return GET(path: "/misc/tags", completionHandler: completionHandler)
    }

    func getMusicMarket(page: Int = 1, completionHandler: @escaping ([String: Any]?, Error?) -> Void) -> URLSessionTask {
        return GET(path: "/arts", params: ["page": String(describing: page), "music": "true"], completionHandler: completionHandler)
    }

    func searchMarket(searchWord: String, page: Int = 1, completionHandler: @escaping ([String: Any]?, Error?) -> Void) -> URLSessionTask {
        return GET(path: "/arts", params: ["page": String(describing: page), "search": searchWord], completionHandler: completionHandler)
    }

    // MARK: - Art

    func getArt(slug: String, completionHandler: @escaping ([String: Any]?, Error?) -> Void) -> URLSessionTask {
        return GET(path: "/arts/\(slug)", completionHandler: completionHandler)
    }

    func recordView(slug: String, completionHandler: ((Error?) -> Void)? = nil) -> URLSessionTask {
        return POST(path: "/arts/\(slug)/rv", completionHandler: { (data: [String: Any]?, error: Error?) in
